...
public class DashboardViewHandler {
    ...
    @StreamListener(KafkaProcessor.INPUT)
    public void whenVidoeProcessed_then_UPDATE_2(
        @Payload VidoeProcessed vidoeProcessed
    ) {
        try {
            if (!vidoeProcessed.validate()) return;
            // view 객체 조회
            Optional<Dashboard> dashboardOptional = dashboardRepository.findById(
                Long.valueOf(vidoeProcessed.getFileid())
            );

            if (dashboardOptional.isPresent()) {
                Dashboard dashboard = dashboardOptional.get();
                // view 객체에 이벤트의 eventDirectValue 를 set 함
                dashboard.setUrl(vidoeProcessed.getUrl());
                // view 레파지 토리에 save
                dashboardRepository.save(dashboard);
            }
        } catch (Exception e) {
